...
# Configuration
input_file = "mcs2024_all_world_production_usgs.csv"
output_file = "mcs2024_all_world_production_usgs_cleaned.csv"
...
# Pattern: anything ending with _2022, _2023, _2022e, _2023e, etc.
df['year'] = df['type'].str.extract(r'_(\d{4}e?)$')

# Remove year suffix from type to get the base metric name
df['type_base'] = df['type'].str.replace(r'_\d{4}e?$', '', regex=True)

print("Sample data with extracted year:")
print(df[['source', 'country', 'type', 'type_base', 'year', 'value', 'commodity']].head(10))
print()
...
# Pivot the data
# Group by: source, country, commodity, type_base
# Pivot on: year_column
# Values: value
df_pivoted = df.pivot_table(
    index=['source', 'country', 'commodity', 'type_base'],
    columns='year_column',
    values='value',
